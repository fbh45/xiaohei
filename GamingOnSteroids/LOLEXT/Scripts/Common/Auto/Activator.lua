...
function __Activator:__init()
	...
	self.ActivatorMenu = MenuElement({type = MENU, id = "Activator", name = "启动器"})
	self.ActivatorMenu:MenuElement({id = "Summoners", name = "召唤师", type = MENU})
	
	self.ActivatorMenu:MenuElement({id = "Healing", name = "治疗", type = MENU})
	self.ActivatorMenu.Healing:MenuElement({id = "Auto", name = "使用治疗物品", value = true, toggle = true})
	self.ActivatorMenu.Healing:MenuElement({id = "Life", name = "％生命", value = 50, min = 1, max = 100, step = 1})
	
	
	self.ActivatorMenu:MenuElement({id = "Cleanse", name = "净化", type = MENU})
	self.ActivatorMenu.Cleanse:MenuElement({id = "CC", name = "控制设置", type = MENU})	
	for _, cc in pairs(self.CCInfo) do
		self.ActivatorMenu.Cleanse.CC:MenuElement({id = cc.Type, name = cc.Name, value = cc.Value})
	end
	self.ActivatorMenu.Cleanse:MenuElement({id="Combo", name="仅在走砍中使用", value = true})
	
	
	self.ActivatorMenu:MenuElement({id = "Damage", name = "伤害", type = MENU})
	self.ActivatorMenu.Damage:MenuElement({id = "AAReset", name = "提亚马特/九头蛇", type = MENU})
	self.ActivatorMenu.Damage.AAReset:MenuElement({id="Killsteal", name="斩杀", value = true})
	self.ActivatorMenu.Damage.AAReset:MenuElement({id="Combo", name="AA重置（走砍）", value = true})
	
	self.ActivatorMenu.Damage:MenuElement({id = "Ranged", name = "比尔吉沃特弯刀/海克斯科技枪刃", type = MENU})
	self.ActivatorMenu.Damage.Ranged:MenuElement({id="Killsteal", name = "斩杀", value = true})
	self.ActivatorMenu.Damage.Ranged:MenuElement({id="Radius", name= "虚弱敌人[走砍]", value = 300, min = 50, max = 600, step = 25})
	...
	self.SummonerFunctions = {
		["SummonerExhaust"] = { 
			Tick = self.Exhaust,
			Initialize = function () 
				local m = self.ActivatorMenu.Summoners:MenuElement({id = "Exhaust", name = "虚弱", type = MENU})
				m:MenuElement{id = "Radius", name = "虚弱半径", value = 300, min = 100, max = 700, step = 50}
				m:MenuElement{id = "Combo", name = "仅在走砍中使用", value = true}
				m:MenuElement{id = "Active", name = "启用", value = true}
			end
		},
		...
		["SummonerDot"] = { 
			Tick = self.Ignite, 
			Initialize = function () 
				local m = self.ActivatorMenu.Summoners:MenuElement({id = "Ignite", name = "点燃", type = MENU})
				m:MenuElement{id = "Combo", name = "在走砍中使用", value = true}
				m:MenuElement{id = "Killsteal", name = "斩杀", value = true}
			end
		},
		...
		["SummonerHeal"] = { 
			Tick = self.Heal,
			Initialize = function () 
				local m = self.ActivatorMenu.Summoners:MenuElement({id = "Heal", name = "治疗", type = MENU})
				m:MenuElement{id = "Health", name = "Health %", value = 30, min = 1, max = 100, step = 1}
				m:MenuElement{id = "Combo", name = "仅走砍", value = false}
			end
		},
		...
		["SummonerBarrier"] = { 
			Tick = self.Barrier,
			Initialize = function () 
				local m = self.ActivatorMenu.Summoners:MenuElement({id = "Barrier", name = "屏障", type = MENU})
				m:MenuElement{id = "Health", name = "Health %", value = 30, min = 1, max = 100, step = 1}
				m:MenuElement{id = "Combo", name = "仅走砍", value = false}
			end
		},
	...
	self.ItemFunctions = {
		...
		[3190] = {
			name = "钢铁烈阳之匣", 
			OnMenu = function()
				local m = self.ActivatorMenu:MenuElement({id = 3190, name = "钢铁烈阳之匣", type = MENU})
				m:MenuElement{id = "Auto", name = "自动施放", value = true}
				m:MenuElement{id = "Count", name = "最小盟友数", value = 2, min = 1, max = 5, step = 1}
		...
		[3040] = {name = "炽天使之拥", 
			OnMenu = function()
				local m = self.ActivatorMenu:MenuElement({id = 3040, name = "炽天使之拥", type = MENU})
				m:MenuElement{id = "Auto", name = "施放不带走砍", value = true}
				m:MenuElement{id = "Health", name = "Health %", value = 30, min = 1, max = 100, step = 1}
			end,
			OnTick = function(slot)
				local realSlot = slot.Slot
		...
		[3222] = {	name = "米凯尔的坩埚",	
		OnMenu = function()
			local m = self.ActivatorMenu:MenuElement({id = 3222, name = "米凯尔的坩埚", type = MENU})			
			
			m:MenuElement({id = "CC", name = "控制设置", type = MENU})	
			for _, cc in pairs(self.CCInfo) do
				m.CC:MenuElement({id = cc.Type, name = cc.Name, value = cc.Value})
			end
			
			m:MenuElement({id = "Targets", name = "目标设置", type = MENU})
			for i = 1, LocalGameHeroCount() do
				local hero = LocalGameHero(i)
				if hero.isAlly then
			...
			m:MenuElement({id="Combo", name="仅在走砍中使用", value = false})
		end,		
		OnTick = self.CleanseTarget	},
		
		[3905] = {	name = "双生暗影",
		OnMenu = function ()
			local m = self.ActivatorMenu:MenuElement({id = 3905, name = "双生暗影", type = MENU})
			m:MenuElement({id = "Range", name = "最大施放距离", value = 2500, min = 500, max = 4000, step = 100})
			m:MenuElement({id = "Radius", name = "最小距离敌人", value = 500, min = 100, max = 2000, step = 50})
		end,
		OnTick = function(slot)
			local realSlot = slot.Slot
			local spellData = myHero:GetSpellData(realSlot)
		...
		[3107] = {	name = "救赎",
		OnMenu = function ()
			local m = self.ActivatorMenu:MenuElement({id = 3107, name = "救赎", type = MENU})
			...
			m:MenuElement({id = "Count", name = "最小目标数", value = 3, min = 1, max = 5, step = 1})
			m:MenuElement({id = "Auto", name = "施放走砍模式", value = true})
		end,
		OnTick = function(slot)
			local realSlot = slot.Slot
			local spellData = myHero:GetSpellData(realSlot)			
