...
function LoadScript()
	...
	Menu.Skills:MenuElement({id = "Q", name = "[Q]碧波之牢", type = MENU})
	Menu.Skills.Q:MenuElement({id = "Accuracy", name = "走砍精度", value = 3, min = 1, max = 6, step = 1 })	
	Menu.Skills.Q:MenuElement({id = "Auto", name = "自动固定施放", value = true, toggle = true })	
	Menu.Skills.Q:MenuElement({id = "Count", name = "自动对＃个敌人施放", tooltip = "How many targets we need to be able to hit to auto cast", value = 3, min = 1, max = 6, step = 1 })
	Menu.Skills.Q:MenuElement({id = "Mana", name = "最小法力", value = 15, min = 1, max = 100, step = 1 })
	Menu.Skills.Q:MenuElement({id = "Assist", name = "辅助键",value = false,  key = 0x70})
		
	Menu.Skills:MenuElement({id = "W", name = "[W]潮冲击之潮", type = MENU})
	Menu.Skills.W:MenuElement({id = "BounceTargets", name = "目标设置[弹跳]", type = MENU})
	...
	Menu.Skills.W:MenuElement({id = "BounceMana", name = "最小法力[反弹]", value = 25, min = 1, max = 100, step = 5 })
	
	Menu.Skills.W:MenuElement({id = "EmergencyTargets", name = "目标设置[无反弹]", type = MENU})
	for i = 1, LocalGameHeroCount() do
		local hero = LocalGameHero(i)
		if hero and hero.isAlly then
	...
	Menu.Skills.W:MenuElement({id = "EmergencyMana", name = "最小法力[无反弹]", value = 25, min = 1, max = 100, step = 5 })
	
		
	Menu.Skills:MenuElement({id = "E", name = "[E]唤潮之佑", type = MENU})
	Menu.Skills.E:MenuElement({id = "Auto", name = "自动增益盟友", value = true, toggle = true })	
	Menu.Skills.E:MenuElement({id = "Targets", name = "目标", type = MENU})
	for i = 1, LocalGameHeroCount() do
		local hero = LocalGameHero(i)
		if hero and hero.isAlly  then
	...
	Menu.Skills.E:MenuElement({id = "Mana", name = "最小法力", value = 25, min = 1, max = 100, step = 5 })
		
	Menu.Skills:MenuElement({id = "R", name = "[R]怒涛之啸", type = MENU})
	Menu.Skills.R:MenuElement({id = "Count", name = "走砍目标数量", tooltip = "How many targets we need to be able to hit to auto cast", value = 2, min = 1, max = 6, step = 1 })
