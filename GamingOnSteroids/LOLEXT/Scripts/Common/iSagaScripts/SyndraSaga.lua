...
require "MapPosition"
	...
	function()
        ...
        if #_EnemyHeroes > 0 then
            for i = 1, TotalHeroes do
                local hero = _EnemyHeroes[i]
            Saga.KillSteal.rKS:MenuElement({id = hero.charName, name = "使用R："..hero.charName, value = true})
            end
        end
		...
		local orb
		if _G.SDK then
			orbwalkername = "集成电路的奥博沃克"
			orb = _G.SDK
		elseif _G.EOW then
			orb = _G.EOW
			orbwalkername = "EOW"
		elseif _G.GOS then
			orbwalkername = "诺迪·奥博沃克"
			orb = _G.GOS
		else
			orbwalkername = "找不到奥博沃克"
		end
    end
)
...
Saga_Menu = 
function()
	Saga = MenuElement({type = MENU, id = "Syndra", name = "佐贺的苔原:大紫球", icon = SagaIcon})
	MenuElement({ id = "blank", type = SPACE ,name = "版本3 . 2 . 1"})
	--Combo
	...
	Saga.Combo:MenuElement({id = "comboActive", name = "走砍键", key = string.byte(" ")})
	...
	Saga.Harass:MenuElement({id = "harassActive", name = "骚扰键", key = string.byte("C")})
	
	Saga:MenuElement({id = "Clear", name = "清线", type = MENU})
	Saga.Clear:MenuElement({id = "UseQ", name = "Q", value = true})
	Saga.Clear:MenuElement({id = "QCount", name = "使用Q清线", value = 3, min = 1, max = 4, step = 1})
	Saga.Clear:MenuElement({id = "UseW", name = "W [不工作]", value = true})
	Saga.Clear:MenuElement({id = "WCount", name = "在X小兵身上使用W [不工作]", value = 3, min = 1, max = 4, step = 1})
	Saga.Clear:MenuElement({id = "clearActive", name = "清线键", key = string.byte("V")})
    

    Saga:MenuElement({id = "KillSteal", name = "斩杀", type = MENU})
    Saga.KillSteal:MenuElement({id = "rKS", name = "R 斩杀", value = false, type = MENU})
    

	Saga:MenuElement({id = "Lasthit", name = "最后一击", type = MENU})
	Saga.Lasthit:MenuElement({id = "UseQ", name = "Q", value = true})
    Saga.Lasthit:MenuElement({id = "lasthitActive", name = "最后一击键", key = string.byte("X")})
    
    Saga:MenuElement({id = "Auto", name = "自动Q", type = MENU})
    Saga.Auto:MenuElement({id = "useAutoQ", name = "启用", key = string.byte("M"), toggle = true})
    ...
    Saga.Drawings:MenuElement({id = "E", name = "绘制长E范围", type = MENU})
    Saga.Drawings.E:MenuElement({id = "Enabled", name = "启用", value = true})       
    Saga.Drawings.E:MenuElement({id = "Width", name = "宽度", value = 1, min = 1, max = 5, step = 1})
    Saga.Drawings.E:MenuElement({id = "Color", name = "颜色", color = Draw.Color(200, 255, 255, 255)})
