...
local textPos = myHero.dir	
	if not isLoaded then
		TryLoad()
		Draw.Text("艾瑞莉娅菜单游戏30秒内出现", 30, textPos.x + 600, textPos.y + 100, Draw.Color(255, 255, 0, 0))
	return end
...
--MainMenu
self.Menu = MenuElement({type = MENU, id = "Irelia", name = "艾瑞莉娅"})
	
self.Menu:MenuElement({type = MENU, id = "ComboSet", name = "走砍设置"})
	
	--ComboMenu  
	self.Menu.ComboSet:MenuElement({type = MENU, id = "Combo", name = "走砍模式"})
	self.Menu.ComboSet.Combo:MenuElement({name = " ", drop = {"E1, W, R, Q, E2, Q + (Q when kill / almost kill)"}})
	self.Menu.ComboSet.Combo:MenuElement({id = "QLogic", name = "最后[Q]几差点杀死或杀死", key = string.byte("Z"), toggle = true})
	...
	self.Menu.ComboSet.Combo:MenuElement({id = "UseR", name = "[R]单一目标", value = true})
	self.Menu.ComboSet.Combo:MenuElement({id = "UseRCount", name = "自动[R]多个敌人", value = true})	
	self.Menu.ComboSet.Combo:MenuElement({id = "RCount", name = "多个敌人", value = 2, min = 2, max = 5, step = 1})
	self.Menu.ComboSet.Combo:MenuElement({id = "Gap", name = "Gapclose [Q]", value = true})
	self.Menu.ComboSet.Combo:MenuElement({id = "Stack", name = "堆叠被动近目标/小兵", value = true})	
	self.Menu.ComboSet.Combo:MenuElement({id = "Draw", name = "绘制Q逻辑文本", value = true})	
	
	--BurstModeMenu
	self.Menu.ComboSet:MenuElement({type = MENU, id = "Burst", name = "突发模式"})	
	self.Menu.ComboSet.Burst:MenuElement({name = " ", drop = {"If Burst Active then Combo Inactive"}})	
	self.Menu.ComboSet.Burst:MenuElement({id = "Start", name = "使用突发模式", key = string.byte("U"), toggle = true})
	self.Menu.ComboSet.Burst:MenuElement({id = "Lvl", name = "艾瑞莉娅级开始爆发", value = 6, min = 6, max = 18, step = 1})
	self.Menu.ComboSet.Burst:MenuElement({id = "Draw", name = "绘制文字", value = true})	


	self.Menu.ComboSet:MenuElement({type = MENU, id = "Ninja", name = "突进模式"})
	self.Menu.ComboSet.Ninja:MenuElement({id = "Q", name = "对所有标记敌人的Q", key = string.byte("I"), toggle = true})

self.Menu:MenuElement({type = MENU, id = "ClearSet", name = "清除设定"})

	--LaneClear Menu
	self.Menu.ClearSet:MenuElement({type = MENU, id = "Clear", name = "清除模式"})
	self.Menu.ClearSet.Clear:MenuElement({type = MENU, id = "Last", name = "最后一击"})
	self.Menu.ClearSet.Clear.Last:MenuElement({name = " ", drop = {"Is only active, if AutoQ Off)"}})	
	self.Menu.ClearSet.Clear.Last:MenuElement({id = "UseQ", name = "最后一击[Q]", value = true})	
	self.Menu.ClearSet.Clear:MenuElement({id = "UseW", name = "[W]", value = true})
	self.Menu.ClearSet.Clear:MenuElement({id = "UseItem", name = "使用九头蛇/提亚马特", value = true})	
	self.Menu.ClearSet.Clear:MenuElement({id = "Mana", name = "最小法力", value = 40, min = 0, max = 100, identifier = "%"})
	
	--JungleClear Menu
	self.Menu.ClearSet:MenuElement({type = MENU, id = "JClear", name = "野区清除模式"})
	self.Menu.ClearSet.JClear:MenuElement({id = "UseQ", name = "最后一击[Q]", value = true})	
	self.Menu.ClearSet.JClear:MenuElement({id = "UseW", name = "[W]", value = true})
	self.Menu.ClearSet.JClear:MenuElement({id = "UseItem", name = "使用九头蛇/提亚马特", value = true})	
	self.Menu.ClearSet.JClear:MenuElement({id = "Mana", name = "最小法力", value = 40, min = 0, max = 100, identifier = "%"})

	--LastHitMode Menu
	self.Menu.ClearSet:MenuElement({type = MENU, id = "LastHit", name = "最后一击模式"})
	self.Menu.ClearSet.LastHit:MenuElement({name = " ", drop = {"Is only active, if AutoQ Off)"}})	
	self.Menu.ClearSet.LastHit:MenuElement({id = "UseQ", name = "最后一击[Q]", value = true})	
	self.Menu.ClearSet.LastHit:MenuElement({id = "UseItem", name = "使用九头蛇/提亚马特", value = true})	
	self.Menu.ClearSet.LastHit:MenuElement({id = "Mana", name = "最小法力", value = 40, min = 0, max = 100, identifier = "%"})	
	self.Menu.ClearSet.LastHit:MenuElement({id = "Active", name = "最后一击键", key = string.byte("X")})

	--AutoQ
	self.Menu.ClearSet:MenuElement({type = MENU, id = "AutoQ", name = "自动Q模式"})
	self.Menu.ClearSet.AutoQ:MenuElement({id = "UseQ", name = "自动斩杀小兵", value = true})
	self.Menu.ClearSet.AutoQ:MenuElement({id = "UseItem", name = "使用九头蛇/提亚马特", value = true})	
	self.Menu.ClearSet.AutoQ:MenuElement({id = "Q", name = "自动Q切换键", key = string.byte("T"), toggle = true})
	self.Menu.ClearSet.AutoQ:MenuElement({id = "Mana", name = "最小法力", value = 40, min = 0, max = 100, identifier = "%"})
	self.Menu.ClearSet.AutoQ:MenuElement({id = "Draw", name = "绘制 打开/关闭 文字", value = true})	


	--HarassMenu
self.Menu:MenuElement({type = MENU, id = "Harass", name = "骚扰设置"})	
	
	self.Menu.Harass:MenuElement({id = "UseQ", name = "[Q]逻辑", value = 1, drop = {"Marked + Dash back Minion", "Everytime"}})	
	self.Menu.Harass:MenuElement({id = "UseW", name = "[W]", value = true})
	self.Menu.Harass:MenuElement({id = "UseE", name = "[E]", value = true})


	--KillSteal
self.Menu:MenuElement({type = MENU, id = "ks", name = "抢杀设置"})
...
self.Menu:MenuElement({type = MENU, id = "MiscSet", name = "其他设置"})		

	--Flee
	self.Menu.MiscSet:MenuElement({type = MENU, id = "Flee", name = "逃跑模式"})
	self.Menu.MiscSet.Flee:MenuElement({id = "Q", name = "逃跑[Q]", value = true})	

	--AutoE 
	self.Menu.MiscSet:MenuElement({type = MENU, id = "AutoE", name = "自动E模式"})
	self.Menu.MiscSet.AutoE:MenuElement({id = "UseE", name = "2-5个敌人晕眩", value = true})	
	
	--AutoW Dangerous Spells
	self.Menu.MiscSet:MenuElement({id = "WSet", name = "自动W模式[测试]", type = MENU})
	...
	self.Menu.MiscSet.WSet:MenuElement({id = "UseW", name = "自动[W]危险法术", value = true})
	self.Menu.MiscSet.WSet:MenuElement({id = "BlockList", name = "黑名单", type = MENU})	
	...
	--Prediction
	self.Menu.MiscSet:MenuElement({type = MENU, id = "Pred", name = "预测模式"})
	self.Menu.MiscSet.Pred:MenuElement({id = "PredR", name = "命中率[R]", value = 1, drop = {"Normal", "High", "Immobile"}})
	self.Menu.MiscSet.Pred:MenuElement({id = "PredW", name = "机会[W]", value = 1, drop = {"Normal", "High", "Immobile"}})
	self.Menu.MiscSet.Pred:MenuElement({id = "PredE", name = "机会[E]", value = 1, drop = {"Normal", "High", "Immobile"}})	
 
	--Drawing 
	self.Menu.MiscSet:MenuElement({type = MENU, id = "Drawing", name = "绘图模式"})
	self.Menu.MiscSet.Drawing:MenuElement({id = "DrawQ", name = "画[Q]范围", value = true})
	self.Menu.MiscSet.Drawing:MenuElement({id = "DrawR", name = "画[R]范围", value = true})
	self.Menu.MiscSet.Drawing:MenuElement({id = "DrawE", name = "画[E]范围", value = true})
	self.Menu.MiscSet.Drawing:MenuElement({id = "DrawW", name = "画[W]范围", value = true})
	self.Menu.MiscSet.Drawing:MenuElement({type = MENU, id = "XY", name = "文字位置设置"})	
	self.Menu.MiscSet.Drawing.XY:MenuElement({id = "x", name = "位置：[X]", value = 0, min = 0, max = 1500, step = 10})
	self.Menu.MiscSet.Drawing.XY:MenuElement({id = "y", name = "位置：[Y]", value = 0, min = 0, max = 860, step = 10})	
...
function Irelia:Draw()
  if myHero.dead then return end
	...
	local textPos = myHero.dir	
	if not FileExist(COMMON_PATH .. "GamsteronPrediction.lua") then
		Draw.Text("请按2x F6重新加载", 50, textPos.x + 100, textPos.y - 250, Draw.Color(255, 255, 0, 0))
	end	
	
	if self.Menu.ComboSet.Burst.Draw:Value() then
		Draw.Text("突发模式：", 15, self.Menu.MiscSet.Drawing.XY.x:Value(), self.Menu.MiscSet.Drawing.XY.y:Value()+30, Draw.Color(255, 225, 255, 0))
		if self.Menu.ComboSet.Burst.Start:Value() then
			if myHero.levelData.lvl >= self.Menu.ComboSet.Burst.Lvl:Value() then
				Draw.Text("活性", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+74, self.Menu.MiscSet.Drawing.XY.y:Value()+30, Draw.Color(255, 0, 255, 0))
			else
				Draw.Text("水平过低", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+74, self.Menu.MiscSet.Drawing.XY.y:Value()+30, Draw.Color(255, 255, 0, 0)) 
			end
		else
			Draw.Text("关闭", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+74, self.Menu.MiscSet.Drawing.XY.y:Value()+30, Draw.Color(255, 255, 0, 0)) 
		end
	end

	if self.Menu.ClearSet.AutoQ.UseQ:Value() and self.Menu.ClearSet.AutoQ.Draw:Value() then 
		Draw.Text("自动[Q]小兵：", 15, self.Menu.MiscSet.Drawing.XY.x:Value(), self.Menu.MiscSet.Drawing.XY.y:Value()+15, Draw.Color(255, 225, 255, 0))
		if self.Menu.ClearSet.AutoQ.Q:Value() then 
			Draw.Text("打开", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+96, self.Menu.MiscSet.Drawing.XY.y:Value()+15, Draw.Color(255, 0, 255, 0))
		else
			Draw.Text("关闭", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+96, self.Menu.MiscSet.Drawing.XY.y:Value()+15, Draw.Color(255, 255, 0, 0)) 
		end	
	end	
	
	Draw.Text("突进模式：", 15, self.Menu.MiscSet.Drawing.XY.x:Value(), self.Menu.MiscSet.Drawing.XY.y:Value()+45, Draw.Color(255, 225, 255, 0))	
	if self.Menu.ComboSet.Ninja.Q:Value() then 

		Draw.Text("打开", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+74, self.Menu.MiscSet.Drawing.XY.y:Value()+45, Draw.Color(255, 0, 255, 0))
	else
		Draw.Text("关闭", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+74, self.Menu.MiscSet.Drawing.XY.y:Value()+45, Draw.Color(255, 255, 0, 0)) 
		
	end	

	if self.Menu.ComboSet.Combo.Draw:Value() then
		Draw.Text("最后[Q]走砍模式：", 15, self.Menu.MiscSet.Drawing.XY.x:Value()-3, self.Menu.MiscSet.Drawing.XY.y:Value(), Draw.Color(255, 225, 255, 0))
		if self.Menu.ComboSet.Combo.QLogic:Value() then
			Draw.Text("差点杀死", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+132, self.Menu.MiscSet.Drawing.XY.y:Value(), Draw.Color(255, 0, 255, 0))
		else
			Draw.Text("杀", 15, self.Menu.MiscSet.Drawing.XY.x:Value()+132, self.Menu.MiscSet.Drawing.XY.y:Value(), Draw.Color(255, 0, 255, 0)) 
		end	
	end		
end
